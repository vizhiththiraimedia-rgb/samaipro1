<?php
// =========================================================================
// SAM AI - Service Configuration: Paper Sam
// Auto-generated: 2026-09-05T03:42:25.996293
// =========================================================================

define('ENVIRONMENT', 'production');
define('SAMAI_API_BASE', 'https://samai.com');
define('SERVICE_NAME', 'Paper Sam');
define('SERVICE_SLUG', 'paper-sam');
define('SERVICE_KEY', 'svc_paper-sam_f23ba0396b4b');
define('SERVICE_ENDPOINT_PREFIX', '/research');

// Theme
define('THEME_COLOR', '#1e40af');
define('THEME_COLOR_HOVER', '#0a2c9b');

// Site Info
define('SITE_TITLE', 'Paper Sam');
define('SITE_TAGLINE', 'AI research paper analysis and writing');
define('SITE_DOMAIN', 'https://paper-sam.sam.ai');

// Credit Costs (from api_registry.json)
define('CREDIT_COST_ANALYZE', 5);
define('CREDIT_COST_WRITE', 6);


// Session
session_start();

// API Client
require_once __DIR__ . '/includes/api-client.php';

$sam_config = [
    'api_base' => SAMAI_API_BASE,
    'service_key' => SERVICE_KEY,
    'jwt_token' => null,
];
$api = new SamAI_API_Client($sam_config);
$session = new SamAI_Session();

if ($session->isLoggedIn()) {
    $api->setToken($session->getToken());
}

function redirect($url) {
    header("Location: " . $url);
    exit;
}

function jsonResponse($data, $status = 200) {
    header('Content-Type: application/json');
    http_response_code($status);
    echo json_encode($data);
    exit;
}

function formatCredits($credits) {
    return number_format($credits);
}
?>
